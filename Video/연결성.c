...
void fullfill(char [][row], int, int, int);
	...
	for (int i = 0; i < 256; i++) 
		for (int j = 0; j < 256; j++) {
			if (i == 0 || i == row - 1 || j == 0 || j == row - 1)
				...
				l[i][j] = Writebuf[i][j] == 255 ? -1 : 0;
		}
	
	for (int i = 1; i < 50; i++) {
		for (int j = 1; j < 50; j++) {
			if (l[i][j] == -1) {
				fullfill(l, i, j, label);
				label++;
			}
		}
	}
...
void fullfill(char buf[][row], int i, int j, int label) {
	if (buf[i][j] == -1) {
		buf[i][j] = label;
		fullfill(buf, i - 1, j, label);
		fullfill(buf, i + 1, j, label);
		fullfill(buf, i, j-1, label);
		fullfill(buf, i, j+1, label);
	}
}